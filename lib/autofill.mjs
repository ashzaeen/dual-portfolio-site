...
async function omdbLookup(title, yearHint) {
...
async function processSeriesRow(row, existingShows) {
  ...
  if (OMDB_API_KEY) {
    try {
      ...
      const data = await omdbLookup(title, yearHint);
      ...
      if (!readText(row.properties?.Runtime) && data.Runtime && data.Runtime !== "N/A") {
        props.Runtime = { rich_text: [{ text: { content: `${data.Runtime} avg` } }] };
      }
...
async function tick() {
  try {
    await Promise.all([
      ...
      tickWriting(),
      tickPinboard(),
      tickHeroSimple("hero chips", HERO_CHIPS_DB),
      tickHeroSimple("hero stats", HERO_STATS_DB),
      tickHeroSimple("hero ticker", HERO_TICKER_DB),
      tickSluggableDB("project", PROJECTS_DB),
      tickSluggableDB("experience", EXPERIENCES_DB),
      tickProjectMedia(),
      tickHeroSimple("education", EDUCATION_DB),
      tickHeroSimple("certification", CERTIFICATIONS_DB),
      tickHeroSimple("coursework", COURSEWORK_DB),
      tickHeroSimple("curiosity", CURIOSITY_DB),
      tickHeroSimple("techstack-category", TECHSTACK_CATS_DB),
      tickHeroSimple("techstack-skill", TECHSTACK_SKILLS_DB),
      tickHeroSimple("footer-social", FOOTER_SOCIALS_DB),
      tickLiveStatus(),
    ]);
  } catch (e) {
    console.log(`[${new Date().toLocaleTimeString()}] poll error: ${e.message}`);
  }
}

console.log(`watch-notion: polling Locations + Stories + Music + Series + Writing + Hero (Chips/Stats/Ticker) + Projects + Project Media + Experiences + Credentials (Education/Certifications/Coursework/Curiosity) + TechStack (Categories/Skills) + Footer Socials + Live Status Update trigger every ${POLL_INTERVAL_MS / 1000}s. Ctrl+C to stop.`);
