...
export async function fetchForecast(coordsString) {
  ...
  try {
    const url =
      ...
      `&hourly=temperature_2m,weather_code&forecast_days=1&timezone=auto&temperature_unit=fahrenheit`;
    ...
    for (let i = 0; i < times.length; i += 2) {
      ...
      const temp = temps[i] != null ? `${Math.round(temps[i])}°F` : "—";
...
function buildBatchPrompt(config, tz, forecastLines) {
  ...
  return [
    ...
    `- SPREAD the go-live times across the whole active day — roughly 07:00 through just past midnight — so the status feels current at any hour and consecutive slots are never more than ~3 hours apart. Weight toward whenever the personal context implies activity (e.g. more late-night/early-hours slots for an all-nighter).`,
    `- Each status should reflect that time slot's FORECASTED weather + temperature and whatever the personal context implies is happening then.`,
    `- Times are 24-hour "HH:MM" in local time, strictly increasing across the list.`,
    `- Each status is ONE sentence, in the exact voice and rules of the system prompt.`,
    `- IMPORTANT: Each status MUST contain the literal token [TIME] exactly once, placed naturally — the site replaces it with the live local clock (e.g. "It's [TIME] and ..."). Do NOT use any OTHER bracketed or ALL-CAPS placeholder tokens (no [LOCATION], [WEATHER], [DEGREE], etc.) — write the actual forecasted weather and temperature in °F as plain words.`,
    ``,
    `Return ONLY JSON: {"statuses":[{"time":"HH:MM","text":"... [TIME] ..."}, ...]}`,
  ].join("\n");
}
